...
<template>
  <Card class="items-center justify-center border-2 border-dashed shadow-none">
    <CardContent class="flex flex-col items-center gap-4 py-8">
      ...
      <!-- Empty State -->
      <p v-if="!dataUrl && !isGenerating" class="text-sm text-muted-foreground">
        {{ $t('qr.preview.empty') }}
      </p>
      ...
      <template v-else-if="isGenerating">
        ...
        <p class="text-sm text-muted-foreground">{{ $t('qr.preview.generating') }}</p>
      </template>

      <!-- QR Code Display -->
      <template v-else>
        <div class="rounded-lg bg-white p-4 shadow-sm">
          <img :src="dataUrl!" alt="QR Code" class="h-auto max-h-80 w-auto max-w-80" />
        </div>
        ...
        <div class="flex gap-2">
          ...
          <Button variant="outline" @click="handleCopy">
            ...
            {{ isCopied ? $t('qr.preview.copied') : $t('qr.preview.copy') }}
          </Button>
        </div>
      </template>
